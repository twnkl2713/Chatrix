...
function addMessage(room, username, message, push = false) {
  ...
  if (STATE.room == room) {
    ...
    node.querySelector(".text").textContent = escapeHtml(message);
...
function setupDeleteRoomButtons() {
  roomListDiv.querySelectorAll(".room-container").forEach(container => {
    const roomButton = container.querySelector(".room");
    const deleteButton = container.querySelector(".delete-room");
    const roomName = roomButton?.dataset.name;

    deleteButton?.addEventListener("click", (e) => {
      e.stopPropagation(); // prevent switching rooms

      if (!roomName || !confirm(`Delete room "${roomName}"?`)) return;

      fetch(`/room/${encodeURIComponent(roomName)}`, {
        method: "DELETE"
      }).then(res => {
        if (res.ok) {
